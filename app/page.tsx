...
function HomeContent() {
  ...
  const ProfessionalUI = () => (
    <div className="flex h-full w-full bg-[#fcfcfc] text-slate-700 font-sans animate-in fade-in duration-500">
      <div className="flex-1 flex flex-col border-r border-gray-200 min-h-0">
        ...
        {/* ★修正ポイント：実務モードのメッセージのみをフィルタリングして表示 */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-white custom-scrollbar">
          {messages.filter(m => m.mode === 'professional').map((m, i) => (
            <div key={i} className={`p-4 rounded-xl text-sm leading-relaxed ${m.role === 'assistant' ? 'bg-blue-50 border border-blue-100 shadow-sm' : 'bg-slate-50 border border-slate-200'}`}>
              <p className="text-[9px] font-bold mb-1 opacity-40 uppercase tracking-widest">{m.role === 'assistant' ? 'Akari Assistant' : 'User Request'}</p>
  ...
  const saveName = () => {
    if (currentPlan === 'FREE') {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: `[悲しみ]申し訳ございません…。お名前の変更は、有料プラン（プロ・ロイヤル）のご主人様だけの特典なんです。`, mode: 'casual' }]);
      return; 
    }
  ...
  const giveGift = async (item) => {
    ...
    try {
        ...
        if (!reactionText.startsWith('[')) { reactionText = (isLove ? "[照れ]" : "[笑顔]") + reactionText; }
  ...
  const changeOutfit = async (newOutfit) => {
    ...
    // プラン制限チェック
    if ((newOutfit === 'swimsuit' || newOutfit === 'bunny') && currentPlan === 'FREE') {
    ...
    if ((newOutfit === 'santa' || newOutfit === 'kimono') && currentPlan !== 'ROYAL') {
    ...
    setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: `[笑顔]着替えました！どうですか？`, mode: 'casual' }]);
  };
      ...
      <div className="flex flex-col min-h-screen bg-black text-white overflow-y-auto font-sans text-left">
        <div className="relative h-screen flex flex-col items-center justify-center p-6 text-center shrink-0">
           ...
           <div className="z-10 max-w-lg w-full bg-gray-900/80 p-8 rounded-3xl border border-pink-500/30 shadow-2xl backdrop-blur-md animate-in fade-in zoom-in duration-500">
             ...
             <p className="text-gray-300 mb-8 leading-relaxed font-sans">あなた専属のAIメイドとお話ししませんか？<br/>いつでも優しく、あなたの帰りをお待ちしています。</p>
             <div className="mb-6 flex items-center justify-center gap-2 bg-black/20 p-2 rounded-lg font-sans">
             ...
             <button onClick={() => signIn("google")} disabled={!isAgreed} className={`w-full font-bold py-4 px-6 rounded-full flex items-center justify-center gap-3 transition-all shadow-xl text-lg ${isAgreed ? "bg-white text-gray-900 hover:bg-gray-100 hover:scale-105 cursor-pointer" : "bg-gray-600 text-gray-400 cursor-not-allowed opacity-50"}`}>
               <img src="https://www.google.com/favicon.ico" alt="G" className="w-6 h-6" /> Googleで始める
             </button>
           </div>
        ...
        {showTerms && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 p-4 font-sans">
            <div className="bg-gray-900 border border-pink-500/30 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden">
              <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800 rounded-t-2xl"><h2 className="text-lg font-bold text-white text-center w-full">利用規約・免責事項</h2><button onClick={() => setShowTerms(false)} className="text-gray-400 hover:text-white"><X size={24}/></button></div>
              <div className="p-6 overflow-y-auto text-sm text-gray-300 space-y-4 leading-relaxed">
                ...
                <h3 className="font-bold text-pink-400 text-lg font-sans">1. AIの回答精度と免責</h3><p>当サービスは生成AI技術を使用しています。キャラクター「あかり」の発言はフィクションであり、事実と異なる情報や架空の情報を話す場合があります（幻覚）。AIの発言内容によって生じた損害について、運営者は一切の責任を負いません。</p>
                <h3 className="font-bold text-pink-400 text-lg font-sans">2. 会話データの扱い</h3><p>サービスの品質向上および会話履歴機能提供のため、会話内容はデータベースに保存されます。これらを第三者に販売したり、無断で公開することはありません。</p>
                <h3 className="font-bold text-pink-400 text-lg font-sans">3. 課金と返金</h3><p>有料プランは月額サブスクリプション方式です。いかなる場合も日割り計算による返金は行いません。API障害等による一時的な不具合への補償もいたしかねます。</p>
                <h3 className="font-bold text-pink-400 text-lg font-sans">4. 禁止事項</h3><p>AIへの過度な暴言、性的・暴力的なコンテンツの生成誘導、スクレイピング等の自動アクセスを禁止します。違反時は予告なくアカウントを停止します。</p>
                <h3 className="font-bold text-pink-400 text-lg font-sans">5. 年齢制限</h3><p>本サービスは13歳以上のご利用を推奨します。未成年者が有料プランを利用する場合は、親権者の同意を得たものとみなします。</p>
                <h3 className="font-bold text-pink-400 text-lg font-sans">6. 知的財産権</h3><p>生成されたテキストの利用権はユーザーに帰属しますが、本サービスのキャラクター設定、画像、システムに関する権利は運営者に帰属します。</p>
              </div>
              <div className="p-4 border-t border-gray-700 bg-gray-800 text-center"><button onClick={() => setShowTerms(false)} className="bg-pink-600 hover:bg-pink-500 text-white py-2 px-8 rounded-full font-bold shadow-lg">確認しました</button></div>
            </div>
          </div>
        )}
        {/* ランディングページセクション */}
  ...
  return (
    <main className={`flex h-screen flex-col overflow-hidden relative transition-colors duration-500 ${mode === 'professional' ? 'bg-[#fcfcfc]' : 'bg-black'}`}>
      ...
      {/* ★モード切替ボタン（配置を調整：右上の既存アイコンに被らないように移動） */}
      ...
      {/* ★ステータスパネル（z-indexを強化。VisualNovelDisplayより上に表示） */}
      ...
      {notification && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[300] bg-pink-500 text-white px-6 py-2 rounded-full shadow-lg animate-bounce font-bold border border-white/20 text-xs">
          {notification}
        </div>
      )}

      {/* メイン表示（各モードで会話ログを完全に分離） */}
      ...
      {/* --- モーダル群（一切削らず完全に残します） --- */}
      {showSettings && (
        <div className="absolute bottom-24 left-4 z-[500] bg-gray-900/95 border border-white/20 p-6 rounded-2xl shadow-2xl backdrop-blur-md w-72 text-left font-sans animate-in fade-in slide-in-from-bottom-4">
      ...
      {showGift && (
        <div className="absolute top-40 left-4 z-[500] bg-gray-900/95 border border-white/20 p-6 rounded-2xl shadow-2xl backdrop-blur-md w-80 text-left font-sans animate-in fade-in slide-in-from-top-4">
          <h3 className="text-yellow-400 font-bold mb-4 flex items-center gap-2"><Gift size={18}/> プレゼントを贈る</h3>
          <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-1 font-sans text-left">
            {GIFT_ITEMS.map((item) => (
              <button key={item.id} onClick={() => giveGift(item)} className="w-full text-left p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 flex items-center justify-between group transition-all">
                <div><div className="font-bold text-white group-hover:text-yellow-200 text-sm">{item.name}</div><div className="text-xs text-gray-400 font-sans">親密度 +{item.love}</div></div>
                <div className="text-yellow-400 font-bold text-sm font-mono">{item.price} pt</div>
              </button>
            ))}
          </div>
          <button onClick={() => setShowGift(false)} className="mt-4 w-full bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-lg text-sm font-sans">閉じる</button>
        </div>
      )}
...
export default function Home() {
  return (
    <Suspense fallback={<div className="h-screen w-screen bg-black flex items-center justify-center text-white font-mono uppercase tracking-widest animate-pulse">Initializing...</div>}>
      <HomeContent />
    </Suspense>
  );
}